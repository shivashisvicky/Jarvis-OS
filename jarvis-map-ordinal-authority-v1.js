...
const mapActive=()=>{const nav=document.querySelector('.nav[data-app="maps"]');return nav instanceof HTMLElement&&nav.classList.contains('selected')};
const results=()=>{if(!mapActive())return [];const m=get(),c=ctx();if(Array.isArray(m?.results)&&m.results.length)return m.results;if(String(c?.domain||'').toUpperCase()==='MAPS'&&Array.isArray(c?.results)&&c.results.length)return c.results;return Array.isArray(window.__JARVIS_MAP_RESULTS__)?window.__JARVIS_MAP_RESULTS__:[]};
const pinpoint=p=>{if(!p)return false;try{window.jarvisContextEngine?.set?.({domain:'MAPS',active:true,location:p.display||p.name||null,selected:{...p}},'merge')}catch{};const f=document.querySelector('#mapFrame');if(!(f instanceof HTMLElement))return false;const lat=Number(p.lat),lon=Number(p.lon);if(!Number.isFinite(lat)||!Number.isFinite(lon))return false;const z=.012;const box=`${lon-z},${lat-z},${lon+z},${lat+z}`;f.style.display='block';f.innerHTML=`<iframe title="JARVIS map for ${String(p.name||'selected result').replace(/"/g,'&quot;')}" loading="eager" style="border:0;width:100%;height:100%;min-height:280px" src="https://www.openstreetmap.org/export/embed.html?bbox=${encodeURIComponent(box)}&layer=mapnik&marker=${encodeURIComponent(`${lat},${lon}`)}"></iframe>`;try{window.dispatchEvent(new CustomEvent('jarvis:map-selected',{detail:{...p,index:results().indexOf(p)}}))}catch{};try{f.scrollIntoView({behavior:'smooth',block:'center'})}catch{};return true};
const run=q=>{const i0=ordinal(q);if(i0===null||!mapActive())return false;const rs=results();const i=i0===-1?Math.max(0,rs.length-1):i0;const p=rs[i];if(!p?.name)return false;stop();window.__JARVIS_PENDING_MAP_SELECTION__={index:i,result:{...p},at:Date.now(),source:'maps-ordinal'};if(pinpoint(p)){say(`Opening ${clean(p.name)}.`);return true}say(`I could not pinpoint ${clean(p.name)} on the map.`);return true};
window.jarvisMapOrdinalAuthority={version:'1.3.0',run};
