...
const stop=()=>{clearWatchdog();const r=recognition;recognition=null;if(r){r.onresult=null;r.onspeechend=null;r.onerror=null;r.onend=null;releaseMic(r)}setState(false)};
...
const start=e=>{const t=e.target instanceof Element?e.target.closest('#voiceBtn'):null;if(!t)return;e.preventDefault();e.stopImmediatePropagation();if(recognition){stop();return}primeSpeechFromGesture();const r=new C();recognition=r;r.lang='en-GB';r.interimResults=true;r.continuous=false;r.maxAlternatives=3;r.onstart=()=>{setState(true);clearWatchdog();stopTimer=window.setTimeout(()=>{if(recognition===r)stop()},8000)};r.onspeechend=()=>{if(recognition===r)window.setTimeout(()=>{if(recognition===r)stop()},80)};r.onresult=ev=>{let final='';for(let i=ev.resultIndex;i<ev.results.length;i++)if(ev.results[i].isFinal)final+=ev.results[i][0].transcript;if(final.trim()){const text=final.trim();stop();submitTranscript(text)}};r.onerror=()=>stop();r.onend=()=>{if(recognition===r){clearWatchdog();recognition=null;setState(false)}};try{r.start()}catch{stop()}};
...
window.addEventListener('jarvis:voice-command',()=>stop(),true);window.addEventListener('pagehide',stop);window.addEventListener('beforeunload',stop);document.addEventListener('visibilitychange',()=>{if(document.hidden)stop()});
})();
