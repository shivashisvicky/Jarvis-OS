...
def search_video(query: str) -> dict:
    ...
    opts = [sys.executable, "-m", "yt_dlp", "--no-playlist", "--quiet", "--no-warnings",
            "--format", "best[ext=mp4][vcodec!=none][acodec!=none]/best[ext=mp4]/best",
            "--js-runtimes", "deno", "--print", "%(id)s\t%(title)s\t%(webpage_url)s\t%(url)s\t%(duration)s", target]
    try:
        result = subprocess.run(opts, cwd=ROOT, capture_output=True, text=True, timeout=45, check=False)
    ...
    return {"id": video_id, "title": title, "webpageUrl": webpage_url, "mediaUrl": media_url,
            "duration": int(duration) if duration.isdigit() else 0}


class Handler(BaseHTTPRequestHandler):
    server_version = "JarvisVideoDemo/3.0"
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/search":
            ...
            if not query:
                self.send_json(400, {"error": "query is required"}); return
            try: self.send_json(200, {"result": search_video(query)})
        ...
        if parsed.path == "/api/media":
            ...
            try:
                ...
                request = urllib.request.Request(target, headers={"User-Agent": "Mozilla/5.0", "Accept": "*/*"})
                if self.headers.get("Range"): request.add_header("Range", self.headers["Range"])
                ...
                self.send_json(502, {"error": f"media proxy failed: {exc}"}); return

        if parsed.path == "/health":
            self.send_json(200, {"ok": True, "service": "jarvis-video-demo", "version": 3}); return
