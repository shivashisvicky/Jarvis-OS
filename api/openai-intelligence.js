export default async function handler(req, res) {
  ...
  const system = [
    ...
    spatial ? 'For Spatial Planner requests, act as a semantic 3D scene planner. Understand the real-world object first, then express it using only the primitive operation contract supplied by the user prompt. The renderer supports box, cylinder, sphere, cone, torus and lathe. Assemblies must be decomposed into multiple primitives when that is required for recognition. Never collapse a multi-part real-world object into one large box merely because its overall envelope is rectangular.' : '',
    ...
    spatial ? 'Use box for panels, boards, shelves, tabletops, cabinet/wardrobe sides, tops, bottoms, backs, frames and other rectangular structural parts. Use cylinder for wheels, axles, rods, pipes, handles and genuinely cylindrical parts. Use sphere or cone only when physically appropriate. Use torus for ring-shaped geometry such as tires. Use lathe for rotationally symmetric curved solids such as a fuel tank, vessel, knob or other smooth turned/form-profiled body; represent a lathe with dimensions {profile:[[radius,height],...]} using at least 6 numeric profile points and keep the profile coherent and non-self-intersecting.' : '',
    ...
    spatial ? 'Return only the requested JSON object. Use only the allowed operations and fields. All dimensions and positions must be numeric SI metres. For lathe, dimensions must contain a numeric profile array of [radius,height] points. Never output markdown, prose outside the JSON object, executable code or unsupported geometry types.' : ''
  ].filter(Boolean).join(' ');
  ...
  const bicyclePlanValid = (plan) => {
    ...
    const wheels = creates.filter(o => /wheel/i.test(String(o.name || '')) && (o.type === 'cylinder' || o.type === 'torus'));
    if (wheels.length < 2) return false;
    return wheels.every(o => o.type === 'torus' ? Number(o.dimensions?.radius) > Number(o.dimensions?.tube || 0) * 4 : Number(o.dimensions?.radius) > Number(o.dimensions?.height || 0) * 2);
  };
