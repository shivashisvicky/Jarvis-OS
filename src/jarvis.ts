...
export function runCommand(raw:string,telemetry:Telemetry):JarvisResult {
 ...
 if(intent==='web' && /\b(news|headlines|current events|latest events)\b/i.test(q)) { window.dispatchEvent(new CustomEvent('jarvis:news',{detail:{query:q}})); return{intent,reply:'Opening the live news desk.'}; }
 switch(intent){
