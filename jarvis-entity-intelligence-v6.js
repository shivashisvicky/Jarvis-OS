...
const CACHE='jarvis:entity-intelligence:v6:';
...
const fetchJson=async(url,ms=10000)=>{const c=new AbortController();const t=setTimeout(()=>c.abort(),ms);try{const r=await fetch(url,{signal:c.signal,cache:'no-store',headers:{Accept:'application/json'}});if(!r.ok)throw new Error(`HTTP ${r.status}`);return await r.json()}finally{clearTimeout(t)}};
...
const gutenberg=async entity=>{try{
 ...
 if(parts.length>=2){const reversed=[...parts].reverse().join(', ');if(!queries.some(q=>q.toLowerCase()===reversed.toLowerCase()))queries.push(reversed)}
 if(parts.length>=3){const surname=parts[parts.length-1];if(!queries.some(q=>q.toLowerCase()===surname.toLowerCase()))queries.push(surname)}
 trace('gutenberg-request',{entity,queries});
 const responses=await Promise.allSettled(queries.map(q=>fetchJson(`https://gutendex.com/books/?search=${encodeURIComponent(q)}&languages=en`,10000)));
 const rows=[];
 for(const response of responses){if(response.status!=='fulfilled')continue;for(const b of (Array.isArray(response.value?.results)?response.value.results:[])){if(!rows.some(x=>x.id===b.id))rows.push(b)}}
