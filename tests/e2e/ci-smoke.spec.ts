...
test.describe('JARVIS media smoke contract', () => {
  test('shell boots with one current video provider and no canned results', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('.brand')).toContainText('J.A.R.V.I.S');
    await expect(page.locator('script[src*="jarvis-video-provider.js"]')).toHaveCount(1);
