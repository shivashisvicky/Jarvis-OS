...
test.describe('JARVIS CI smoke contract', () => {
  ...
  test('video authority does not invent a result when live indexes fail', async ({ page }) => {
    await page.route('https://pipedapi.tokhmi.xyz/search**', route => route.abort());
    await page.route('https://pipedapi.moomoo.me/search**', route => route.abort());
    await page.route('https://piped-api.garudalinux.org/search**', route => route.abort());
    await page.route('https://api.piped.privacydev.net/search**', route => route.abort());
    await page.route('https://www.youtube.com/results**', route => route.abort());
    await page.goto('/');
    await page.locator('button.nav[data-app="media"]').click();
    await page.locator('#videoQuery').fill('unreachable test query');
    await page.locator('#videoSearch').click();
    await expect(page.locator('#videoResults')).toContainText('NO LIVE RESULTS');
