...
test('media keyword search uses a provider response and plays the selected ID', async ({ page }) => {
  const result = { type:'video', title:'Provider cats result', videoId:'M7lc1UVf-VE', author:'JARVIS Provider', videoThumbnails:[{ quality:'medium', url:'https://i.ytimg.com/vi/M7lc1UVf-VE/hqdefault.jpg' }] };
  await page.route('**/search**', async route => {
    if (/piped|invidious/i.test(route.request().url())) {
      await route.fulfill({ status:200, contentType:'application/json', body:JSON.stringify({ items:[result], ...result }) });
      return;
    }
  ...
  await expect(page.locator('.jvc-card')).toHaveCount(1, { timeout:10000 });
