...
const extract=raw=>{
 ...
 if(currentLocation.test(q)){
   const explicit=q.match(/\b(?:in|at|for)\s+(.+?)(?:\s+(?:today|right now))?\s*[?!.,]*$/i)?.[1]?.trim();
   return explicit||'current location';
 }
 const p=q.replace(weatherWords,' ').replace(/\b(what(?:\s+is|['’]s|s)?|how|today|right now|like)\b/gi,' ').replace(/^\s*(in|at|for|of)\s+/i,'').replace(/\s+/g,' ').trim();
...
const wrap=()=>{
 ...
 window.dispatchEvent=event=>{
   try{if(event?.type==='jarvis:weather'&&event.detail){event.detail={...event.detail,place:extract(event.detail.place||event.detail.text||'')}}}catch{}
...
const fixCommandInput=()=>{
 ...
 form.addEventListener('submit',event=>{
   const q=input.value||'';
   if(!/\b(weather|temperature|forecast|how hot|how cold)\b/i.test(q)||!currentLocation.test(q))return;
   event.preventDefault();
   event.stopImmediatePropagation();
   try{window.dispatchEvent(new CustomEvent('jarvis:weather',{detail:{place:extract(q)}}))}catch{}
 },true);
};
