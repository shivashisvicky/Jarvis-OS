(()=>{
  ...
  const localSearch=q=>{
    ...
    const base=hits.length?hits:INDEX.filter(v=>/trending|india/i.test(v.tags));
    return base.slice(0,4).map(v=>({...v,source:'JARVIS LOCAL INDEX',thumb:`https://i.ytimg.com/vi/${v.id}/hqdefault.jpg`}));
  };
  ...
  const publicSearch=async q=>{
    const enc=encodeURIComponent(q);
    const tasks=PROVIDERS.map(base=>base.includes('piped')
      ? fetchJson(`${base}/search?q=${enc}&filter=videos&region=IN`).then(d=>(Array.isArray(d)?d:d?.items||[]).map(v=>normalize(v,'PUBLIC INDEX')).filter(Boolean))
      : fetchJson(`${base}/api/v1/search?q=${enc}&type=video&page=1&region=IN`).then(d=>(Array.isArray(d)?d:[]).map(v=>normalize(v,'PUBLIC INDEX')).filter(Boolean))
    );
    const settled=await Promise.allSettled(tasks);const out=[];const seen=new Set();
    for(const s of settled)if(s.status==='fulfilled')for(const v of s.value||[])if(!seen.has(v.id)){seen.add(v.id);out.push(v)}
    return out.slice(0,8);
  };
  const search=async q=>{
    const local=localSearch(q);
    try{
      const remote=await publicSearch(q);
  ...
  function mount(){
    ...
    if(!input||!button||!results||!player)return false;
    ...
    const replaceButton=()=>{const current=$('#videoSearch');if(!current||current.dataset.jvi==='1')return;const fresh=current.cloneNode(true);fresh.dataset.jvi='1';current.replaceWith(fresh);fresh.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();void run(input.value)},true)};
    replaceButton();
    ...
    const playButton=$('#playVideo');if(playButton&&playButton.dataset.jvi!=='1'){playButton.dataset.jvi='1';playButton.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();const raw=$('#videoUrl')?.value.trim()||'';const id=youtubeIdFromInput(raw);if(id)play({id,title:'Pasted YouTube video'});else if(/^https?:\/\//i.test(raw)&&/\.(mp4|webm|ogg)(\?.*)?$/i.test(raw)){player.innerHTML=`<video controls playsinline src="${E(raw)}"></video>`;set('PLAYING · DIRECT MEDIA')}},true)}
