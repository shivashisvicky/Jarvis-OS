(() => {
  ...
  const idOf = raw => {
    ...
    try {
      ...
      if (u.hostname.endsWith('youtube.com')) return u.searchParams.get('v') || (() => { const p=u.pathname.split('/').filter(Boolean); const i=p.findIndex(x=>['shorts','embed','live'].includes(x)); return i>=0?p[i+1]:null; })();
    } catch {}
    return null;
  };
  ...
  const fetchJson = async url => {
    ...
    try { const r=await fetch(url,{signal:c.signal,cache:'no-store',headers:{Accept:'application/json'}}); if(!r.ok) throw Error(String(r.status)); return r.json(); }
  ...
  async function searchVideos(q){
    ...
    if(!items.length)throw Error('No videos');
    return items;
  }
  ...
  async function stream(item){
    const bases=[item.gateway,...INVIDIOUS,...PIPED].filter(Boolean);
    return Promise.any([...new Set(bases)].map(async b=>{
      if(b.includes('piped')){
        ...
        const s=a.find(x=>/video\/mp4/i.test(x.mimeType||''))||a[0]; if(!s?.url)throw Error('no stream');
      ...
      const s=a[0]; if(!s?.url)throw Error('no stream');
  ...
  function css(){
    ...
    const s=document.createElement('style');s.id='jvcStyle';s.textContent=`
      ...
      .jvc-card{display:grid;grid-template-columns:150px 1fr 34px;gap:12px;align-items:center;width:100%;padding:0;overflow:hidden;text-align:left;border:1px solid #173545;border-radius:14px;background:rgba(3,11,17,.92);color:#d9f7ff;cursor:pointer}.jvc-card:hover{border-color:#49cfff}.jvc-thumb{width:150px;aspect-ratio:16/9;object-fit:cover;background:#020509}.jvc-info{min-width:0;padding:9px 0}.jvc-info strong{display:block;font-size:.88rem;line-height:1.25}.jvc-info small{display:block;color:#7896a3;margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.jvc-play{color:#5edcff;margin-right:10px}
      ...
      @media(max-width:700px){.jvc-card{grid-template-columns:108px 1fr 30px}.jvc-thumb{width:108px}.jvc-info strong{font-size:.8rem}}
  ...
  function install(){
    ...
    // main.ts attaches its media listeners in a setTimeout(0). Wait until that
    // work has happened before replacing the nodes, otherwise both controllers
    // would attach to the same result list.
  ...
  function boot(){
    // Poll briefly because the main app creates the media DOM first and binds
    // its own async transport immediately afterwards. Installing too early was
    // the source of the old double-controller behaviour.
