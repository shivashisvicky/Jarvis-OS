(() => {
  ...
  // JARVIS is a static site, so it cannot safely depend on a single unofficial
  // YouTube proxy. Use several live gateways for in-shell results, then fall
  // back to the real YouTube/Bing search pages. Direct playback uses the
  // official YouTube player when a proxy stream is unavailable.
  const PIPED = ['https://pipedapi.kavin.rocks','https://pipedapi.tokhmi.xyz','https://pipedapi.syncpundit.io','https://pipedapi.moomoo.me'];
  ...
  const normalize = (x, source, gateway) => {
    const id = x.videoId || idOf(x.url);
    if (!id) return null;
    return {id,title:x.title || 'Untitled video',author:x.author || x.uploader || x.uploaderName || 'YouTube',views:x.viewCountText || x.views || (x.viewCount ? `${Number(x.viewCount).toLocaleString()} views` : ''),duration:x.lengthSeconds ? duration(x.lengthSeconds) : (x.duration || ''),published:x.publishedText || x.uploadedDate || '',thumbnail:thumb(x),source,gateway};
  };

  async function searchVideos(q) {
    try {
      const {data,url} = await race(PIPED.map(b => `${b}/search?q=${encodeURIComponent(q)}&filter=videos&region=IN`));
      const raw = Array.isArray(data) ? data : (data.items || []), items = raw.map(x => normalize(x,'PIPED',new URL(url).origin)).filter(Boolean);
      if (items.length) return items;
    } catch {}
    try {
      const {data,url} = await race(INVIDIOUS.map(b => `${b}/api/v1/search?q=${encodeURIComponent(q)}&type=video&region=IN&page=1`));
  ...
  async function trending() {
    try {
      const {data,url} = await race(PIPED.map(b => `${b}/trending?region=IN`));
      const items = (Array.isArray(data) ? data : []).map(x => normalize(x,'PIPED',new URL(url).origin)).filter(Boolean);
      if (items.length) return items;
    } catch {}
    try {
      const {data,url} = await race(INVIDIOUS.map(b => `${b}/api/v1/trending?region=IN&type=default`));
      const items = (Array.isArray(data) ? data : []).map(x => normalize(x,'INVIDIOUS',new URL(url).origin)).filter(Boolean);
      if (items.length) return items;
    } catch {}
    return [];
  ...
  async function stream(item) {
    ...
    return Promise.any([...new Set(bases)].map(async b => {
      if (b.includes('piped')) {
        const d = await fetchJson(`${b}/streams/${encodeURIComponent(item.id)}`);
        const a = (d.videoStreams || []).filter(x => x.url && !x.videoOnly).sort((x,y) => (y.height || 0) - (x.height || 0));
        const s = a.find(x => /video\/mp4/i.test(x.mimeType || '')) || a[0];
  ...
  function install() {
    ...
    const openSearch = (provider,q) => { const query=encodeURIComponent(q || input.value.trim() || 'videos'); const url=provider==='bing' ? `https://www.bing.com/videos/search?q=${query}` : `https://www.youtube.com/results?search_query=${query}`; window.open(url,'_blank','noopener,noreferrer'); };
    ...
    const showLiveSearch = q => { results.innerHTML=`<div class="jvc-status">LIVE SEARCH READY · CHOOSE A SEARCH ENGINE</div><div class="jvc-live-grid jvc-actions"><button class="primary" id="jvcYoutube">YOUTUBE SEARCH</button><button id="jvcBing">BING VIDEO SEARCH</button></div>`; setStatus('LIVE SEARCH READY'); results.querySelector('#jvcYoutube').onclick=()=>openSearch('youtube',q); results.querySelector('#jvcBing').onclick=()=>openSearch('bing',q); };
    const doSearch = async () => { const q=input.value.trim(); if(!q){setStatus('READY · ENTER A VIDEO SEARCH TERM');return;} const token=++generation; results.innerHTML='<div class="jvc-status">SEARCHING LIVE VIDEO SOURCES…</div>'; setStatus(`SEARCHING · ${q}`); const items=await searchVideos(q); if(token!==generation)return; if(items.length)render(items,items[0]?.source || 'VIDEO SEARCH'); else showLiveSearch(q); };
    const searchButton=document.querySelector('#videoSearch'); if(searchButton){const b=searchButton.cloneNode(true);searchButton.replaceWith(b);b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();void doSearch();},true);}
    input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();e.stopImmediatePropagation();void doSearch();}},true);
    document.querySelectorAll('[data-video-provider]').forEach(old=>{const b=old.cloneNode(true);old.replaceWith(b);b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();const provider=b.dataset.videoProvider;if(provider==='trending')void trending().then(items=>items.length?render(items,'INDIA TRENDING'):openSearch('youtube','trending videos India'));else if(provider==='bing')openSearch('bing',input.value.trim() || 'trending videos');else openSearch('youtube',input.value.trim() || 'trending videos');},true);});
    const playButton=document.querySelector('#playVideo'); if(playButton){const b=playButton.cloneNode(true);playButton.replaceWith(b);b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();const raw=document.querySelector('#videoUrl')?.value?.trim()||'',id=idOf(raw);if(id)play({id,title:'YouTube video',author:'YouTube',thumbnail:`https://i.ytimg.com/vi/${id}/hqdefault.jpg`,source:'direct'});else if(/^https?:\/\//i.test(raw)){player.innerHTML=`<video controls autoplay playsinline src="${esc(raw)}"></video>`;setStatus('PLAYING · DIRECT MEDIA URL');}else setStatus('READY · PASTE A YOUTUBE URL OR VIDEO ID');},true);}
    window.jarvisVideoSearch=q=>{input.value=q||'';void doSearch();};
    window.jarvisOpenVideoSearch=(provider,q)=>openSearch(provider==='bing'?'bing':'youtube',q || input.value || 'videos');
    results.innerHTML='<div class="jvc-status">READY · SEARCH ANY TOPIC TO FIND VIDEOS</div>'; setStatus('READY · VIDEO SEARCH ONLINE');
    return true;
  }
