(() => {
  ...
  function styles() {
    ...
    s.id = 'jmc-style';
    s.textContent = `
      .jarvis-mission-console{position:relative;overflow:hidden;margin:0 0 22px;padding:22px;border:1px solid rgba(126,205,255,.16);background:linear-gradient(135deg,rgba(12,25,38,.88),rgba(4,10,16,.94));box-shadow:inset 0 1px rgba(255,255,255,.04),0 18px 60px rgba(0,0,0,.18)}
      .jarvis-mission-console:before{content:'';position:absolute;inset:0;background:linear-gradient(90deg,transparent 0%,rgba(111,213,255,.04) 50%,transparent 100%);pointer-events:none}
      .jmc-head{position:relative;display:flex;justify-content:space-between;gap:20px;align-items:flex-start;margin-bottom:18px}.jmc-head h2{margin:2px 0 4px;letter-spacing:.08em}.jmc-prompt{margin:0;color:var(--muted,#8da0b2);font-size:14px}.jmc-status{display:flex;align-items:center;gap:8px;font-size:10px;letter-spacing:.12em}.jmc-status i{width:7px;height:7px;border-radius:50%;background:#65f0b5;box-shadow:0 0 12px #65f0b5}.jmc-status small{color:var(--muted,#8da0b2)}
      ...
      @media(max-width:850px){.jmc-actions{grid-template-columns:repeat(2,1fr)}}@media(max-width:520px){.jmc-actions{grid-template-columns:1fr}.jmc-head{display:block}.jmc-status{margin-top:12px}}
    `;
  ...
  function mount() {
    ...
    el.innerHTML = `
      <div class="jmc-head"><div><p class="eyebrow">JARVIS / ORCHESTRATION</p><h2>JARVIS INTELLIGENCE CORE</h2><p class="jmc-prompt">What should JARVIS work on?</p></div><div class="jmc-status"><i></i><span>READY</span><small>LOCAL-FIRST</small></div></div>
      <div class="jmc-actions">${ACTIONS.map(([k,t,d], i) => `<button class="jmc-action" data-jmc-index="${i}" type="button"><span class="jmc-index">0${i + 1}</span><span><b>${t}</b><small>${d}</small></span><strong>↗</strong></button>`).join('')}</div>
    ...
    el.querySelectorAll('.jmc-action').forEach(btn => btn.addEventListener('click', () => {
      ...
      else if (i === 1) document.querySelector('[data-app="media"]')?.dispatchEvent(new MouseEvent('click',{bubbles:true}));
      else if (i === 3) document.querySelector('[data-app="api"]')?.dispatchEvent(new MouseEvent('click',{bubbles:true}));
    }));
  }
