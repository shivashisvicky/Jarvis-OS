...
const videoSearchUrl=(q,mode='all')=>`https://m.youtube.com/results?search_query=${encodeURIComponent(String(q||''))}`;
...
async function piped(q){
 const bases=['https://pipedapi.kavin.rocks','https://pipedapi.adminforge.de','https://pipedapi-libre.kavin.rocks'];
 for(const b of bases){try{const d=await json(`${b}/search?q=${encodeURIComponent(q)}&filter=videos&region=IN`,6500);const xs=uniq(Array.isArray(d)?d:d?.items||[]);if(xs.length)return xs}catch{}}
 return [];
}
async function invidious(q,trending=false){
 const bases=['https://inv.nadeko.net','https://invidious.nerdvpn.de','https://invidious.tiekoetter.com'];
 for(const b of bases){try{const u=trending?`${b}/api/v1/trending?region=IN&type=default`:`${b}/api/v1/search?q=${encodeURIComponent(q)}&type=video&region=IN&page=1`;const d=await json(u,6500);const xs=uniq(Array.isArray(d)?d:[]);if(xs.length)return xs}catch{}}
 return [];
}
...
function command(q){const r=$('#jarvisReply');if(!r)return;r.innerHTML='<span class="jv4-command-source">JARVIS knowledge layer · IN-HOUSE</span><br>Searching…';knowledge(q,{style:{},innerHTML:'',setAttribute(){}}).then(()=>{}).catch(()=>{});}
...
// Command answers must render in the dashboard reply, not in the Research panel.
...
document.addEventListener('click',e=>{
 ...
 if(t.matches('.jv4-video-card,.jvc-card')){e.preventDefault();e.stopImmediatePropagation();play(t.dataset.jv4Video||t.dataset.jvcId||'');return}
 if(t.matches('#playVideo')){e.preventDefault();e.stopImmediatePropagation();const id=idFrom($('#videoUrl')?.value||'');if(id)play(id);else mediaState('READY · PASTE A YOUTUBE URL OR VIDEO ID');return}
