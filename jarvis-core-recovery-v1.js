(() => {
  ...
  const ensureHomeEngineeringCards=()=>{const grid=document.querySelector('.module-grid');if(!grid)return;const existing=new Set([...grid.querySelectorAll('[data-app]')].map(x=>x.getAttribute('data-app')));[['api','⇄','ENGINEERING','API Lab','REST request client'],['remote','↔','ENGINEERING','SFTP','Secure transfer workspace']].forEach(([id,icon,group,name,desc])=>{if(existing.has(id))return;const b=document.createElement('button');b.className='module-card';b.type='button';b.dataset.app=id;b.innerHTML=`<span class="module-icon">${icon}</span><div><small>${group}</small><strong>${name}</strong><p>Open ${desc}</p></div><b>›</b>`;grid.appendChild(b);});const bay=document.querySelector('[data-engineering-bay]')||document.createElement('button');if(!bay.parentElement){bay.className='module-card';bay.type='button';bay.dataset.engineeringBay='1';bay.innerHTML='<span class="module-icon">⚙</span><div><small>ENGINEERING</small><strong>Engineering Bay</strong><p>Open engineering intelligence tools</p></div><b>›</b>';grid.appendChild(bay);}if(!bay.dataset.bound){bay.dataset.bound='1';bay.addEventListener('click',()=>document.querySelector('.nav[data-app="api"]')?.click());}};
