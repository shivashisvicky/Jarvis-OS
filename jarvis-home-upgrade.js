(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .jhu-quick button{appearance:none;border:1px solid rgba(74,179,211,.25);border-radius:999px;background:rgba(5,21,29,.86);color:#9bc5d1;padding:8px 11px;font:700 10px/1 inherit;letter-spacing:.04em;cursor:pointer;transition:.16s ease}
  ...
  function build() {
    ...
    hero.innerHTML = `<div class="jhu-hero-grid">
      <div><div class="jhu-kicker">J.A.R.V.I.S / MISSION CONTROL</div><h2 class="jhu-greeting">${greeting()}</h2><p class="jhu-sub">Your command layer is awake. Search the web, navigate, run tools, launch media, or simply tell JARVIS what you need.</p></div>
      <div class="jhu-telemetry">${stat('CORE', online ? 'NOMINAL' : 'DEGRADED', true)}${stat('LOCAL TIME', now.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}))}${stat('CPU', cores)}${stat('MEMORY', memory)}</div>
    </div>`;
    command.parentNode.insertBefore(hero, command);

    const quick = document.createElement('div');
    quick.id = 'jhuQuick';
    quick.className = 'jhu-quick';
    quick.innerHTML = `<button data-cmd="Give me today's top news">TODAY'S BRIEF</button><button data-cmd="Search the web for the latest AI news">AI INTEL</button><button data-app="maps">OPEN MAPS</button><button data-app="media">OPEN MEDIA</button><button data-cmd="What can you do?">CAPABILITIES</button>`;
    hero.insertAdjacentElement('afterend', quick);
    quick.querySelectorAll('[data-cmd]').forEach(button => button.addEventListener('click', () => runCommand(button.getAttribute('data-cmd') || '')));
    ...
    section.className = 'jhu-section';
    section.innerHTML = '<strong>LIVE SYSTEM PULSE</strong><span>LOCAL DEVICE TELEMETRY</span>';
    command.insertAdjacentElement('afterend', section);

    const pulse = document.createElement('div');
    pulse.className = 'jhu-pulse';
    ...
    const refresh = () => {
      const h = document.querySelector('#jhuHomeHero .jhu-greeting');
      const t = document.querySelector('#jhuHomeHero .jhu-stat:nth-child(2) strong');
      if (h) h.innerHTML = greeting();
  ...
  const tryBuild = () => {
    if (document.querySelector('#commandForm')) build();
  };
  tryBuild();
