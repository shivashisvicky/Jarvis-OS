(()=>{
'use strict';
if(window.__JARVIS_COMMAND_AUTHORITY_HOTFIX_V13__)return;
window.__JARVIS_COMMAND_AUTHORITY_HOTFIX_V13__=true;
...
const handleMapFollowup=q=>{
 ...
 reply(`The nearest option is ${clean(best?.name||best?.title||best?.display_name)}${distanceText}.`);
 return true;
};
const handle=q=>{
 const s=clean(q);
