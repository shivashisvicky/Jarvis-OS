...
window.fetch=async function(input,init){
 ...
 if(intelligenceSource.test(url)){
  let body={};try{body=JSON.parse(String(init?.body||'{}'))}catch{}
  const raw=String(body?.query||'');const m=raw.match(/User request:\s*([\s\S]*)$/i);const q=(m?m[1]:raw).trim();
  if(simpleDesk.test(q)){const plan=deskPlan();return new Response(JSON.stringify({plan,text:JSON.stringify(plan),model:'local-spatial-simple-assembly',provider:'local'}),{status:200,headers:{'Content-Type':'application/json'}})}
 }
 if(!spatialSource.test(url))return nativeFetch(input,init);
 const res=await nativeFetch(input,init);if(!res.ok)return res;const code=await res.text();return new Response(patch(code),{status:res.status,statusText:res.statusText,headers:res.headers})
};
})();
