...
function patch(code){
 ...
 const precisionPercent=q.match(/(\\d+(?:\\.\\d+)?)\\s*%/i);
 ...
 if(/\\b(selected|current)\\b/.test(precisionLow)&&/\\b(shorter|taller|thinner|thicker|wider|narrower|deeper|shallower)\\b/.test(precisionLow)){
  if(precisionPercent){
   const p=Number(precisionPercent[1])/100,d={};
   if(/shorter|taller/.test(precisionLow))d.height=/shorter/.test(precisionLow)?-p:p;
   else if(/thinner|thicker/.test(precisionLow)){const s=/thinner/.test(precisionLow)?-p:p;d.width=s;d.depth=s}
   else if(/wider|narrower/.test(precisionLow))d.width=/narrower/.test(precisionLow)?-p:p;
   else if(/deeper|shallower/.test(precisionLow))d.depth=/shallower/.test(precisionLow)?-p:p;
...
`;
 if(code.includes(localNeedle)&&!code.includes('const precisionPercent='))code=code.replace(localNeedle,localPatch);
