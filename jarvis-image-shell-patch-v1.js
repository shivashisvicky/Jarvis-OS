(() => {
  ...
  function install() {
    const group = document.querySelector('.rail .nav-group'); if (group && !group.querySelector('[data-jarvis-image-open]')) group.insertAdjacentHTML('beforeend', navButton());
    const grid = document.querySelector('.module-grid'); if (grid && !grid.querySelector('[data-jarvis-image-open]')) grid.insertAdjacentHTML('beforeend', cardButton());
    document.querySelectorAll('[data-jarvis-image-open]').forEach(el => {
      if (el.dataset.visionBound) return; el.dataset.visionBound = '1';
      el.addEventListener('click', event => { event.preventDefault(); event.stopImmediatePropagation(); void openVision(); }, true);
    });
  }
  async function openVision() {
    try {
      await loadAssets();
      const workspace = document.querySelector('#workspace'); if (!workspace) throw new Error('JARVIS workspace is unavailable.');
      ...
      const init = window.jarvisInitImageStudio; if (typeof init !== 'function') throw new Error('Vision Studio entry point is unavailable.'); init();
    } catch (error) {
      const status = document.querySelector('#workspace'); if (status) status.insertAdjacentHTML('beforeend', `<section class="panel info-card"><h3>Vision Lab unavailable</h3><p>${String(error?.message || 'Could not load Vision Lab.')}</p></section>`);
    }
  }
