(()=>{
'use strict';
if(window.__JARVIS_CONTEXT_ENGINE_V2__)return;
window.__JARVIS_CONTEXT_ENGINE_V2__=true;
...
const setContext=(patch={},policy='merge')=>{if(policy==='replace')reset();if(policy==='inherit'&&!active())return false;Object.keys(patch||{}).forEach(k=>{if(patch[k]!==undefined)state[k]=clone(patch[k])});state.updatedAt=now();state.turn+=1;return true};
const snapshot=()=>({...clone(state),active:active()});
const resolveReference=text=>{const q=String(text||'').trim().toLowerCase();if(!active())return {matched:false,reason:'no_context'};if(/^(?:the\s+)?first(?:\s+one)?$/.test(q))return {matched:!!state.results?.[0],type:'RESULT',index:0,value:state.results?.[0]||null};if(/^(?:the\s+)?second(?:\s+one)?$/.test(q))return {matched:!!state.results?.[1],type:'RESULT',index:1,value:state.results?.[1]||null};if(/^(?:the\s+)?third(?:\s+one)?$/.test(q))return {matched:!!state.results?.[2],type:'RESULT',index:2,value:state.results?.[2]||null};if(/^(?:the\s+)?last(?:\s+one)?$/.test(q)){const i=(state.results?.length||0)-1;return {matched:i>=0,type:'RESULT',index:i,value:i>=0?state.results[i]:null}}if(/^(?:there|that place|that location)$/.test(q)&&state.location)return {matched:true,type:'LOCATION',value:state.location};if(/^(?:it|that|this)$/.test(q)&&state.entity)return {matched:true,type:'ENTITY',value:state.entity};return {matched:false,reason:'unresolved'}};
