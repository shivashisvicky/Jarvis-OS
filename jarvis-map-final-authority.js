(()=>{
'use strict';
if(window.__JARVIS_MAP_FINAL_AUTHORITY_V3__)return;
window.__JARVIS_MAP_FINAL_AUTHORITY_V3__=true;
...
const cacheKey=q=>`jarvis-map-v3:${clean(q).toLowerCase()}`;
...
const nominatim=async(query,contextual=false)=>{
 const cached=readCache((contextual?'context:':'exact:')+query);if(cached)return cached;
 try{
  const u=new URL('https://nominatim.openstreetmap.org/search');
  u.searchParams.set('format','jsonv2');u.searchParams.set('q',contextual?`${query}, Bhubaneswar, Odisha, India`:query);u.searchParams.set('limit','12');u.searchParams.set('addressdetails','1');u.searchParams.set('namedetails','1');u.searchParams.set('countrycodes','in');u.searchParams.set('accept-language','en');u.searchParams.set('dedupe','1');
  const r=await fetch(u.toString(),{cache:'default',headers:{Accept:'application/json'}});if(!r.ok)return[];
  const data=await r.json();const out=(Array.isArray(data)?data:[]).map(mapResult).filter(p=>Number.isFinite(p.lat)&&Number.isFinite(p.lon));writeCache((contextual?'context:':'exact:')+query,out);return out;
 }catch{return[]}
};
const rank=(q,places)=>{const lower=clean(q).toLowerCase();return places.map(p=>{const text=`${p.name} ${p.display_name}`.toLowerCase(),words=wordHits(q,text);return{...p,__score:(text.includes(lower)?100:0)+(p.name.toLowerCase()===lower?80:0)+words*20+(['amenity','shop','tourism','leisure'].includes(p.category)?10:0)}}).sort((a,b)=>b.__score-a.__score).slice(0,8).map(({__score,...p})=>p)};
