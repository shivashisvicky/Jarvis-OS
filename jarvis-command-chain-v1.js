...
const GUT='(?:\\s+in\\s+(?:gutenberg|standard\\s+ebooks?))?';
const splitChain=text=>{
 const s=clean(text);
 if(!s)return [];
 const direct=s.match(new RegExp('^(.+?)\\s+((?:open|read|show|select|choose|play|watch)\\s+(?:the\\s+)?'+ORD+'(?:\\s+(?:one|result))?'+GUT+')$','i'));
 ...
 const ordinal=new RegExp('^(?:the\\s+)?'+ORD+'(?:\\s+(?:one|result))?'+GUT+'$','i');
 const actionOrdinal=new RegExp('^(?:open|read|show|select|choose|play|watch)\\s+(?:the\\s+)?'+ORD+'(?:\\s+(?:one|result))?'+GUT+'$','i');
 const m=s.match(new RegExp('^(.+?)\\s+((?:open|read|show|select|choose|play|watch)\\s+(?:the\\s+)?'+ORD+'(?:\\s+(?:one|result))?'+GUT+')$','i'));
 if(m&&clean(m[1])&&actionOrdinal.test(clean(m[2])))return[clean(m[1]),clean(m[2])];
 const n=s.match(new RegExp('^(.+?)\\s+((?:the\\s+)?'+ORD+'(?:\\s+(?:one|result))?'+GUT+')$','i'));
...
const inferType=q=>{
 ...
 if(new RegExp('\\b(?:open|read|show|select|choose|play|watch)\\b.*\\b(?:'+ORD+'|it|that|this)\\b'+GUT+'$','i').test(s))return 'CONTEXT_FOLLOWUP';
...
const canonicalFollowup=clause=>clean(clause).replace(/\s+in\s+(?:gutenberg|standard\s+ebooks?)\s*$/i,'').trim();
const canonicalFirst=clause=>clean(clause).replace(/^(?:please\s+)?(?:let\s+me\s+)?identify\s+/i,'').trim();
const dispatchFollowup=clause=>{const target=canonicalFollowup(clause);try{if(window.jarvisContextOrdinalExtension?.run?.(target))return true}catch{}try{if(window.jarvisContextReferenceAuthority?.run?.(target))return true}catch{}return dispatchClause(target)};
const dispatchFirst=async(clause,resolved)=>{const target=canonicalFirst(clause);try{const ebook=window.jarvisEbookBookFastResolver;if(typeof ebook?.run==='function'&&await ebook.run(target))return true}catch{}const canEntity=typeof window.jarvisEntityAuthority?.handle==='function'&&typeof window.jarvisEntityAuthority?.candidate==='function'&&window.jarvisEntityAuthority.candidate(target);if(canEntity&&(!resolved||resolved.type==='BOOKS'))return !!(await window.jarvisEntityAuthority.handle(target));return dispatchClause(clause,{deferMediaPlay:false})};
...
const interceptSubmit=e=>{const form=e.target;if(!(form instanceof HTMLFormElement)||form.id!=='commandForm')return;if(window.__JARVIS_COMMAND_CHAIN_RUNNING__&&!window.__JARVIS_COMMAND_CHAIN_INTERNAL_SUBMIT__){e.preventDefault();e.stopImmediatePropagation();return}if(window.__JARVIS_COMMAND_CHAIN_RUNNING__)return;const input=form.querySelector('#commandInput');if(!(input instanceof HTMLInputElement))return;const parsed=parse(input.value);if(!parsed)return;e.preventDefault();e.stopImmediatePropagation();window.dispatchEvent(new CustomEvent('jarvis:command-chain',{detail:{parts:parsed.parts,routes:parsed.routes,version:'2.2.7'}}));void runChain(parsed)};
const interceptVoice=e=>{if(window.__JARVIS_COMMAND_CHAIN_RUNNING__){e.preventDefault();e.stopImmediatePropagation();return}const text=clean(e.detail?.text);if(!text)return;const parsed=parse(text);if(!parsed)return;e.preventDefault();e.stopImmediatePropagation();const input=document.querySelector('#commandInput');if(input instanceof HTMLInputElement)input.value=text;window.dispatchEvent(new CustomEvent('jarvis:command-chain',{detail:{parts:parsed.parts,routes:parsed.routes,version:'2.2.7',source:'voice'}}));void runChain(parsed)};
document.addEventListener('submit',interceptSubmit,true);window.addEventListener('jarvis:voice-command',interceptVoice,true);window.jarvisCommandChain={version:'2.2.7',parse};
})();
