...
const ORD='(?:\\d+(?:st|nd|rd|th)?|first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh|twelfth|thirteenth|fourteenth|fifteenth|sixteenth|seventeenth|eighteenth|nineteenth|twentieth|twenty-first|twenty-second|twenty-third|twenty-fourth|twenty-fifth|twenty-sixth|twenty-seventh|twenty-eighth|twenty-ninth|thirtieth)';
const splitChain=text=>{
 const s=clean(text);
 if(!s)return [];
 const direct=s.match(new RegExp('^(.+?)\\s+((?:open|read|show|select|choose|play|watch)\\s+(?:the\\s+)?'+ORD+'(?:\\s+(?:one|result))?)$','i'));
 if(direct&&clean(direct[1]))return[clean(direct[1]),clean(direct[2])];
 const parts=s.split(new RegExp('(?:,\\s*|\\s+(?:and|then)\\s+)(?=(?:open|read|show|select|choose|play|watch|take|navigate|go|find|search|look\\s+up|lookup|google|tell|give|what|what\\'s|what\\s+is)\\b|(?:the\\s+)?'+ORD+'\\b|(?:it|that|this)\\b)','i')).map(normalize).filter(Boolean);
 if(parts.length>=2)return parts;
 const ordinal=new RegExp('^(?:the\\s+)?'+ORD+'(?:\\s+(?:one|result))?$','i');
 const actionOrdinal=new RegExp('^(?:open|read|show|select|choose|play|watch)\\s+(?:the\\s+)?'+ORD+'(?:\\s+(?:one|result))?$','i');
 const m=s.match(new RegExp('^(.+?)\\s+((?:open|read|show|select|choose|play|watch)\\s+(?:the\\s+)?'+ORD+'(?:\\s+(?:one|result))?)$','i'));
 if(m&&clean(m[1])&&actionOrdinal.test(clean(m[2])))return[clean(m[1]),clean(m[2])];
 const n=s.match(new RegExp('^(.+?)\\s+((?:the\\s+)?'+ORD+'(?:\\s+(?:one|result))?)$','i'));
...
const inferType=q=>{
 ...
 if(/\b(?:open|read|show|select|choose|play|watch)\b.*\b(?:\d+(?:st|nd|rd|th)?|first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh|twelfth|thirteenth|fourteenth|fifteenth|sixteenth|seventeenth|eighteenth|nineteenth|twentieth|it|that|this|the)\b/i.test(s))return 'CONTEXT_FOLLOWUP';
 if(new RegExp('^(?:(?:the\\s+)?'+ORD+'|it|that|this)\\b','i').test(s))return 'CONTEXT_FOLLOWUP';
...
const dispatchFollowup=clause=>{try{if(window.jarvisContextOrdinalExtension?.run?.(clause))return true}catch{}try{if(window.jarvisContextReferenceAuthority?.run?.(clause))return true}catch{}return dispatchClause(clause)};
...
const interceptSubmit=e=>{const form=e.target;if(!(form instanceof HTMLFormElement)||form.id!=='commandForm')return;if(window.__JARVIS_COMMAND_CHAIN_RUNNING__&&!window.__JARVIS_COMMAND_CHAIN_INTERNAL_SUBMIT__){e.preventDefault();e.stopImmediatePropagation();return}if(window.__JARVIS_COMMAND_CHAIN_RUNNING__)return;const input=form.querySelector('#commandInput');if(!(input instanceof HTMLInputElement))return;const parsed=parse(input.value);if(!parsed)return;e.preventDefault();e.stopImmediatePropagation();window.dispatchEvent(new CustomEvent('jarvis:command-chain',{detail:{parts:parsed.parts,routes:parsed.routes,version:'2.2.4'}}));void runChain(parsed)};
const interceptVoice=e=>{if(window.__JARVIS_COMMAND_CHAIN_RUNNING__){e.preventDefault();e.stopImmediatePropagation();return}const text=clean(e.detail?.text);if(!text)return;const parsed=parse(text);if(!parsed)return;e.preventDefault();e.stopImmediatePropagation();const input=document.querySelector('#commandInput');if(input instanceof HTMLInputElement)input.value=text;window.dispatchEvent(new CustomEvent('jarvis:command-chain',{detail:{parts:parsed.parts,routes:parsed.routes,version:'2.2.4',source:'voice'}}));void runChain(parsed)};
document.addEventListener('submit',interceptSubmit,true);window.addEventListener('jarvis:voice-command',interceptVoice,true);window.jarvisCommandChain={version:'2.2.4',parse};
})();
