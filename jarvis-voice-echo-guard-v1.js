(()=>{
'use strict';
if(window.__JARVIS_VOICE_ECHO_GUARD_V5__)return;
window.__JARVIS_VOICE_ECHO_GUARD_V5__=true;
...
const markTts=()=>{
  ...
  // IMPORTANT: TTS must never cancel TTS. Only release the microphone/recognition
  // session so the utterance that is about to be spoken is left untouched.
  try{window.jarvisStopVoiceRecognitionOnly?.()}catch{}
};
const installSpeechGuard=()=>{
  const synth=window.speechSynthesis;
  if(!synth||typeof synth.speak!=='function'||synth.speak.__jarvisTtsGuard)return;
  const original=synth.speak.bind(synth);
  const wrapped=function(utterance){
    markTts();
    try{
      const oldEnd=utterance?.onend;
      if(utterance)utterance.onend=function(event){
...
const guard=e=>{
  ...
  if(exactEcho){
    e.preventDefault?.();e.stopImmediatePropagation?.();markTts();
...
const timer=window.setInterval(installSpeechGuard,250);window.setTimeout(()=>window.clearInterval(timer),20000);
})();
