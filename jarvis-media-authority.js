...
(() => {
  ...
  class JarvisMediaAuthority {
    constructor() {
      ...
      this.observeUntilReady();
    }

    observeUntilReady() {
      if (this.tryInit()) return;
      this.observer = new MutationObserver(() => {
        if (this.tryInit() && this.observer) {
    ...
    tryInit() {
      ...
      this.container = this.resolve([
        '#media-center', '[data-app-container="media"]', '.media-center-root', '#media', 'section[data-app="media"]'
      ]);
      if (!this.container) return false;

      this.searchInput = this.resolve([
        '#videoQuery', 'input[name="videoQuery"]', '.media-search-input', 'input[type="text"]'
      ], this.container);
      this.searchButton = this.resolve([
        '#videoSearch', 'button.search-btn', 'button.media-search-btn', '.media-search-submit', 'button[type="submit"]'
      ], this.container);
      this.resultsContainer = this.resolve([
        '.video-results-container', '#videoResults', '.jyt-results', '.media-results'
      ], this.container) || this.createZone('video-results-container');
      this.playerContainer = this.resolve([
        '.video-player-container', '#videoPlayer', '.jyt-player', '.media-player'
      ], this.container) || this.createZone('video-player-container');
      this.statusLabel = this.resolve(['.video-status', '.media-status', '#videoStatus'], this.container);

      if (!this.searchInput || !this.searchButton) {
        console.error('[JARVIS Media] Media Center exists but search controls are missing.');
        return false;
      }
      this.bindEvents();
    ...
    bindEvents() {
      ...
      this.resultsContainer.addEventListener('click', (event) => {
        ...
        if (embedUrl) this.playVideo(embedUrl, card.getAttribute('data-provider') || 'video', card.getAttribute('data-title') || 'JARVIS Video');
      });
    }
    ...
    async executeSearch(query) {
      ...
      try {
        const winner = await Promise.any(PROVIDERS.map(provider => this.fetchProvider(provider, query)));
    ...
    async fetchProvider(provider, query) {
      ...
      try {
        ...
        if (!data || !Array.isArray(data.data) || data.data.length === 0) throw new Error('Provider returned no results');
        ...
        const results = data.data.map(item => {
          ...
          const embedUrl = this.safeUrl(item.embedUrl || item.embedPath || `/videos/embed/${id}`, provider.baseUrl);
    ...
    setLoadingState(query) {
      ...
      this.status(`SEARCHING · LIVE VIDEO INDEXES · ${query.toUpperCase()}`);
    }

    renderResults(results, providerName) {
      this.clear(this.resultsContainer);
      this.status(`ONLINE · ${results.length} RESULTS VIA ${providerName.toUpperCase()}`);
    ...
    renderDegradedState(query) {
      this.clear(this.resultsContainer);
      this.status('STANDBY · PROVIDERS OFFLINE');
      const wrapper = document.createElement('div');
      wrapper.className = 'media-degraded-state';
      const message = document.createElement('p');
      message.textContent = 'LIVE VIDEO PROVIDERS UNAVAILABLE. AUTHORITATIVE FALLBACK:';
    ...
    playVideo(embedUrl, provider, title) {
      ...
      this.clear(this.playerContainer);
      ...
      this.status(`PLAYING · ${String(provider).toUpperCase()}`);
    }
  ...
  const bootstrap = () => { window.JarvisMedia = new JarvisMediaAuthority(); };
