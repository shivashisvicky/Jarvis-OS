(()=>{
  'use strict';
  if(window.__JARVIS_STOP_VOICE_BUTTON_FIX_V3__)return;
  window.__JARVIS_STOP_VOICE_BUTTON_FIX_V3__=true;
  ...
  scan();
  document.addEventListener('pointerdown',event=>{if(isStopVoiceButton(event.target))hardStop()},true);
  document.addEventListener('touchstart',event=>{if(isStopVoiceButton(event.target))hardStop()},true);
  document.addEventListener('click',event=>{if(isStopVoiceButton(event.target))hardStop()},true);
