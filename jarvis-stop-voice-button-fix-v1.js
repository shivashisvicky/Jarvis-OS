(()=>{
  'use strict';
  if(window.__JARVIS_STOP_VOICE_BUTTON_FIX_V4__)return;
  window.__JARVIS_STOP_VOICE_BUTTON_FIX_V4__=true;
  ...
  const bindButton=button=>{if(!(button instanceof HTMLElement)||button.dataset.jarvisStopBound==='v4')return;button.dataset.jarvisStopBound='v4';const handler=event=>{event.preventDefault();event.stopImmediatePropagation();hardStop()};for(const type of ['pointerdown','pointerup','touchstart','touchend','mousedown','mouseup','click']){try{button.addEventListener(type,handler,{capture:true,passive:type!=='touchstart'&&type!=='touchend'})}catch{}}};
