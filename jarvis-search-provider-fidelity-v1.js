...
const sync=()=>{const provider=document.querySelector('#webProvider')?.value==='brave'?'BRAVE':'BING';const status=document.querySelector('#jwsStatus');if(status&&/RESULTS\s·/i.test(status.textContent||'')&&!/BING\s+FALLBACK/i.test(status.textContent||''))status.textContent=status.textContent.replace(/RESULTS\s·\s*\w+(?:\s+FALLBACK)?/i,`RESULTS · ${provider}`);document.querySelectorAll('#jwsResults .web-result small').forEach(n=>{const t=n.textContent||'';if(t&&t!=='WEB'&&!t.startsWith(provider)){const d=t.indexOf(' · ');n.textContent=provider+(d>=0?t.slice(d):'')}})};
