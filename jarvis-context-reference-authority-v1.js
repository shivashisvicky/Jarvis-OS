...
const getBooksContext=()=>{
 ...
 if(live?.active&&live.domain==='BOOKS'&&Array.isArray(live.results)&&live.results.length)return {ctx:live,source:'live'};
 const memory=window.jarvisContextMemory;
 const saved=memory?.get?.();
 if(saved?.domain==='BOOKS'&&Array.isArray(saved.results)&&saved.results.length)return {ctx:saved,source:'memory'};
 return null;
};
const resolve=(target,ctx,source)=>{
 ...
 if(resolved?.matched&&resolved.domain==='BOOKS')return resolved;
...
window.jarvisContextReferenceAuthority={version:'2.0.0',run};
})();
