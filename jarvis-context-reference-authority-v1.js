...
const getBooksContext=()=>{
 ...
 if(live?.active&&blockingDomains.has(String(live.domain||'').toUpperCase()))return null;
 const last=readLastBook();
 if(last)return {ctx:last,source:'last-book'};
 const saved=window.jarvisContextMemory?.get?.();
 if(saved?.domain&&isBookDomain(saved.domain)&&Array.isArray(saved.results)&&saved.results.length)return {ctx:saved,source:'memory'};
 return null;
};
...
const resolve=(target,ctx,source)=>{
 ...
 if(source==='live'){const r=engine?.resolveReference?.(target);if(r?.matched&&isBookDomain(r.domain))return r}
 if(source==='memory'){const r=window.jarvisContextMemory?.resolveReference?.(target);if(r?.matched&&isBookDomain(r.domain))return r}
...
window.jarvisContextReferenceAuthority={version:'2.5.0',run};
})();
