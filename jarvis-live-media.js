(() => {
  ...
  function loadConfig() {
    ...
    configPromise = new Promise((resolve, reject) => {
      ...
      script.src = `${CONFIG}?v=20260821-youtube-backend-first`;
      script.onload = () => window.JARVIS_YOUTUBE_API_KEY ? resolve(window.JARVIS_YOUTUBE_API_KEY) : reject(new Error('YouTube API key unavailable'));
      script.onerror = () => reject(new Error('YouTube configuration unavailable'));
  ...
  function readCache(query) {
    try {
      ...
      if (!entry?.timestamp || Date.now() - entry.timestamp > CACHE_TTL_MS || !Array.isArray(entry.results)) return null;
      return entry.results;
    } catch { return null; }
  }
  function writeCache(query, results) {
    try { sessionStorage.setItem(cacheKey(query), JSON.stringify({ timestamp: Date.now(), results })); } catch {}
  }
  async function backendSearch(clean) {
    const response = await fetch(`${API_ENDPOINT}?q=${encodeURIComponent(clean)}`, { headers: { Accept: 'application/json' } });
    if (!response.ok) {
      const error = new Error(`YouTube search service ${response.status}`);
  ...
  async function directSearch(clean) {
  ...
  async function fetchResults(clean) {
    try {
      return await backendSearch(clean);
    } catch (error) {
      if (![404, 405, 500, 502, 503, 504].includes(error?.status)) throw error;
      return directSearch(clean);
    }
  }
  function renderResults(results) {
    const host = document.querySelector('#videoResults');
    if (!host) return;
    if (!results.length) throw new Error('No live results');
    installCardStyles();
    host.innerHTML = results.slice(0, MAX_RESULTS).map(item => {
      const id = item.id;
      return `<button type="button" class="jvc-card" data-jvc-id="${esc(id)}"><img loading="lazy" src="${esc(item.thumbnail || '')}" alt=""><span><strong>${esc(item.title || 'YouTube video')}</strong><small>${esc(item.channel || 'YouTube')}</small><span class="play">PLAY</span></span></button>`;
    }).join('');
  }
  async function search(query) {
    ...
    if (cached) {
      ...
      state.textContent = `${cached.length} CACHED RESULTS`;
      return;
    }
    state.textContent = 'LIVE SEARCH';
    results.innerHTML = '<div class="empty">Searching YouTube…</div>';
    const promise = fetchResults(clean).then(items => {
      renderResults(items);
      writeCache(clean, items);
      state.textContent = `${Math.min(items.length, MAX_RESULTS)} LIVE RESULTS`;
      return items;
    }).catch(error => {
      state.textContent = error?.status === 429 ? 'QUOTA LIMITED' : 'DEGRADED';
      results.innerHTML = `<div class="empty">Live YouTube search unavailable. ${esc(error instanceof Error ? error.message : 'Try again.')}</div>`;
      throw error;
    }).finally(() => { if (activeSearch?.key === key) activeSearch = null; });
  ...
  function install() {
    ...
    window.addEventListener('jarvis:media', event => {
      ...
      if (input && query) { input.value = query; void search(query); }
    });
    ...
    const runPending = () => {
      ...
      const input = document.querySelector('#videoQuery');
      if (input) {
        input.value = pendingVoiceQuery;
        const q = pendingVoiceQuery;
        pendingVoiceQuery = '';
        void search(q);
      }
    };
    runPending();
    new MutationObserver(runPending).observe(document.documentElement, { childList: true, subtree: true });
  }
  window.addEventListener('jarvis:media', event => { pendingVoiceQuery = String(event.detail?.query ?? '').trim(); });
