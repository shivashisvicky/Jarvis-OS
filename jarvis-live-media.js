(() => {
  ...
  function cacheKey(query){return `jarvis-youtube:${normalize(query)}`} function readCache(query){try{const raw=sessionStorage.getItem(cacheKey(query));if(!raw)return null;const entry=JSON.parse(raw);if(!entry?.timestamp||Date.now()-entry.timestamp>CACHE_TTL_MS||!Array.isArray(entry.results))return null;return entry.results}catch{return null}} function writeCache(query,results){try{sessionStorage.setItem(cacheKey(query),JSON.stringify({timestamp:Date.now(),results}))}catch{}}
  function readHistory(){try{const value=JSON.parse(localStorage.getItem(HISTORY_KEY)||'[]');return Array.isArray(value)?value.filter(x=>typeof x==='string').slice(0,6):[]}catch{return []}} function writeHistory(query){const clean=String(query||'').trim().replace(/\s+/g,' ');if(!clean)return;try{const next=[clean,...readHistory().filter(x=>normalize(x)!==normalize(clean))].slice(0,6);localStorage.setItem(HISTORY_KEY,JSON.stringify(next));renderHistory(next)}catch{}}
  ...
  function publishContext(query,results,selected=null){try{window.jarvisContextEngine?.set?.({domain:'YOUTUBE',active:true,query:String(query||'').trim()||null,results:Array.isArray(results)?results.map(x=>({...x})):[],selected:selected?{...selected}:null},'merge')}catch{}}
  function renderResults(results){const host=document.querySelector('#videoResults');if(!host)return;if(!results.length)throw new Error('No live YouTube results');currentResults=results.slice(0,MAX_RESULTS);installCardStyles();host.innerHTML=currentResults.map(item=>`<button type="button" class="jvc-card" data-jvc-id="${esc(item.id)}"><img loading="lazy" src="${esc(item.thumbnail||'')}" alt=""><span><strong>${esc(item.title||'YouTube video')}</strong><small>${esc(item.channel||'YouTube')}</small><span class="play">PLAY</span></span></button>`).join('')}
  async function search(query){const state=document.querySelector('#mediaState'),results=document.querySelector('#videoResults');if(!state||!results)return [];const clean=String(query||'').trim(),key=normalize(clean);if(!key)return [];if(activeSearch?.key===key)return activeSearch.promise;if(activeController)activeController.abort();const cached=readCache(clean);if(cached?.length){renderResults(cached);publishContext(clean,currentResults);state.textContent=`${cached.length} CACHED RESULTS`;writeHistory(clean);return cached}state.textContent='LIVE SEARCH';results.innerHTML='<div class="empty">Searching YouTube…</div>';const controller=new AbortController();activeController=controller;const timeout=window.setTimeout(()=>controller.abort(),20000);const promise=backendSearch(clean,controller.signal).then(items=>{if(!items.length)throw new Error('No live YouTube results');renderResults(items);publishContext(clean,currentResults);writeCache(clean,items);state.textContent=`${Math.min(items.length,MAX_RESULTS)} LIVE RESULTS`;return items}).catch(error=>{if(error?.name==='AbortError'){state.textContent='RETRY';results.innerHTML='<div class="empty">The live search took too long. Try again.</div>';return []}state.textContent=error?.status===429?'QUOTA LIMITED':'DEGRADED';results.innerHTML=`<div class="empty">${esc(error?.message||'Live YouTube search unavailable. Try again.')}</div>`;return []}).finally(()=>{window.clearTimeout(timeout);if(activeSearch?.key===key)activeSearch=null;if(activeController===controller)activeController=null});activeSearch={key,promise};return promise}
  function openIndexed(index){const i=Number(index);if(!Number.isInteger(i)||i<0||!currentResults[i]?.id)return false;const nav=document.querySelector('.nav[data-app="media"]');if(nav instanceof HTMLElement&&!nav.classList.contains('selected'))nav.click();let tries=0;const timer=window.setInterval(()=>{tries+=1;if(document.querySelector('#jarvisPlayer')){window.clearInterval(timer);player(currentResults[i].id);publishContext(document.querySelector('#videoQuery')?.value||'',currentResults,currentResults[i]);return}if(tries>=80)window.clearInterval(timer)},50);return true}
  function isDirectUrl(value){return Boolean(videoId(value))}
  async function autoPlayFirst(query){
  ...
  function install(){if(window.__JARVIS_LIVE_MEDIA__)return;window.__JARVIS_LIVE_MEDIA__=true;window.jarvisAutoPlayFirstYouTubeResult=autoPlayFirst;window.jarvisLiveMediaAuthority={openIndexed,getContext:()=>({domain:'YOUTUBE',active:true,query:document.querySelector('#videoQuery')?.value||null,results:currentResults.map(x=>({...x})),selected:null})};renderHistory();window.addEventListener('jarvis:media',event=>{const query=String(event.detail?.query??'').trim();pendingVoiceQuery=query;const input=document.querySelector('#videoQuery');if(input&&query){input.value=query;void search(query)}});document.addEventListener('click',event=>{const button=event.target?.closest?.('#videoSearch');if(!button)return;const input=document.querySelector('#videoQuery'),query=input?.value?.trim()||'';if(!query||isDirectUrl(query))return;event.preventDefault();event.stopImmediatePropagation();void search(query)},true);document.addEventListener('keydown',event=>{if(event.key!=='Enter'||!event.target?.matches?.('#videoQuery'))return;const query=event.target.value.trim();if(!query||isDirectUrl(query))return;event.preventDefault();void search(query)},true);document.addEventListener('click',event=>{const card=event.target?.closest?.('[data-jvc-id]');if(card){const id=card.getAttribute('data-jvc-id');const selected=currentResults.find(x=>String(x.id)===String(id));if(selected)publishContext(document.querySelector('#videoQuery')?.value||'',currentResults,selected);player(id)}});const runPending=()=>{if(!pendingVoiceQuery)return;const input=document.querySelector('#videoQuery');if(input){input.value=pendingVoiceQuery;const q=pendingVoiceQuery;pendingVoiceQuery='';void search(q)}};runPending();new MutationObserver(runPending).observe(document.documentElement,{childList:true,subtree:true})}
