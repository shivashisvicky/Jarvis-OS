(()=>{'use strict';
if(window.__JARVIS_EBOOK_VOICE_BRIDGE_V2__)return;
window.__JARVIS_EBOOK_VOICE_BRIDGE_V2__=true;
...
const handle=e=>{const d=e.detail||{},q=String(d.text||'').trim();if(!q||!ebook(q))return;let reply='';if(d.resolved){const type=String(d.entity?.type||'').toUpperCase(),score=Number(d.entity?.score||0);if(d.resolutionMode!=='bare'||!['BOOK','BOOK_AUTHOR'].includes(type)||score<.9)return;reply=`Searching Gutenberg for ${clean(q)}.`}else if(/\b(?:ebook|ebooks|book|books|novel|novels|gutenberg|standard ebooks|read|reading)\b/i.test(q)){reply=clean(q)?`Searching Gutenberg for ${clean(q)}.`:'Opening the public-domain ebook library.'}else return;
setTimeout(()=>{try{if(native.speaking||native.pending)return}catch{};speakNative(reply)},180)};
