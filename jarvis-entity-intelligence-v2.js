...
const gutenberg=async entity=>{try{
 ...
 const author=rows.filter(b=>(b.authors||[]).some(a=>String(a.name||'').toLowerCase().trim()===q));
 if(exact.length||author.length)return{type:'BOOK',score:.99,source:'gutenberg',entity,results:exact.length?exact:author};
 const authorEvidence=rows.filter(b=>(b.authors||[]).some(a=>authorMatch(entity,a.name)>=.8));
 if(authorEvidence.length)return{type:'BOOK_AUTHOR',score:.97,source:'gutenberg',entity,results:authorEvidence.slice(0,10)};
 const partial=rows.filter(b=>String(b.title||'').toLowerCase().includes(q)||(b.authors||[]).some(a=>String(a.name||'').toLowerCase().includes(q)));
 if(partial.length)return{type:'BOOK_CANDIDATE',score:.76,source:'gutenberg',entity,results:partial.slice(0,10)}
 }catch{}return null};
...
window.jarvisEntityIntelligence=Object.freeze({version:'4.0.0',resolve,candidate});
})();
