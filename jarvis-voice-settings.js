...
const getRate=()=>{try{const stored=Number(localStorage.getItem(KEY));const version=localStorage.getItem(VERSION_KEY);if(version!=='3'&&Number.isFinite(stored)&&stored<=0.95){localStorage.setItem(KEY,String(DEFAULT_RATE));localStorage.setItem(VERSION_KEY,'3');return DEFAULT_RATE}localStorage.setItem(VERSION_KEY,'3');return Number.isFinite(stored)?clamp(stored):DEFAULT_RATE}catch{return DEFAULT_RATE}};
const setRate=v=>{const rate=clamp(v);try{localStorage.setItem(KEY,String(rate));localStorage.setItem(VERSION_KEY,'3')}catch{};return rate};
